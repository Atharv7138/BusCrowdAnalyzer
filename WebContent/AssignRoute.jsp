...
<head>
<meta charset="UTF-8">
<title>View Routes | Bus Depot Panel</title>
...
<div class="header-bar">
    <h4>Route Directory</h4>
    <p>Select a route and assign a bus for operations</p>
</div>
...
try{
    ...
    ps = con.prepareStatement("SELECT * FROM route WHERE demail=?");
...
%>
        <tr>
            <td><%= rs.getInt("id") %></td>
            <td><strong><%= rs.getString("routenumber") %></strong></td>
            <td><%= rs.getString("routename") %></td>
            <td><%= rs.getString("stop") %></td>
            <td><%= rs.getString("startlocation") %></td>
            <td><%= rs.getString("endlocation") %></td>
            <td class="text-center">
                <a href="AssignBusForm.jsp?routename=<%= rs.getString("routename") %>&routenumber=<%= rs.getString("routenumber") %>"
                   class="action-assign">
                    <i class="fas fa-random"></i> Assign Bus
                </a>
            </td>
        </tr>
<%
